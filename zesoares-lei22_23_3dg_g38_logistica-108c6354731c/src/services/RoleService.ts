...
export default class RoleService implements IRoleService {
  ...
  public async getAll(): Promise<Result<IRoleDTO[]>> {
    try {
      const trucks = await this.roleRepo.findAll();

      if (trucks === null) {
        return Result.fail<IRoleDTO[]>("Roles not found");
      } else {
        let listRolesDTOResult: IRoleDTO[] = [];
        trucks.forEach(truck => {
            listRolesDTOResult.push(RoleMap.toDTO(truck) as IRoleDTO);
        });
        return Result.ok<IRoleDTO[]>(listRolesDTOResult);
      }
    } catch (e) {
      throw e;
    }
